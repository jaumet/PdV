...
def analyze(mode, reorder, num_groups, abstention_id=None):
    ...
    """
    # Load data from tsv
    analyzer = Analyzer(pdvdatatmp+"allkey.tsv", abstention_id)
...
def analyze_simpleMar(abstention_id=None):
    ...
    # Load data from tsv
    analyzer = Analyzer(c.pdvdatatmp+"allkey.tsv", abstention_id)
...
def output(map_new, number):
    ...
    for m in map_new:
        try:
            ...
            if is_number(m[1]):
                a = [int(recolocationmap[str(m[0])]), m[1]]
    ...
    # WRITE FILE
    f1 = open(c.pdvdatatmp+'recolocation1.tsv', "w")
...
if __name__ == '__main__':
    ...
    if options.out_fn:
        ...
        shutil.copyfile(fn, c.pdvdatatmplog+"map-%s-%s.tsv" % (715, int(time.time())))
        ...
        print "+++++++++++++++++++++++++++++++++++++"
        #print myoutput
        print "map updated -> number of lines:"
        print len(map_new)
